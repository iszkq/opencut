		...
		try {
			if (fileType === "image") {
				...
				try {
					...
					// Some audio-only MP4/M4A files are reported as video/* by the
					// browser. Mediabunny correctly finds no video track; treat those
					// files as audio rather than sending them to the video renderer.
					if (
						error instanceof Error &&
						error.message === "No video track found in the file"
					) {
						fileType = "audio";
						duration = await getMediaDuration({ file, forceAudio: true });
					} else {
						const message =
							error instanceof Error
								? error.message
...
const getMediaDuration = ({
	...
	return new Promise((resolve, reject) => {
		const element =
			!forceAudio && file.type.startsWith("video/")
