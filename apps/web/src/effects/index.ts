...
export function resolveEffectPasses({
	...
	if (definition.renderer.buildPasses) {
		return definition.renderer.buildPasses({ effectParams, width, height, localTime, duration });
	}
	return definition.renderer.passes.map((pass) => ({
		shader: pass.shader,
		uniforms: pass.uniforms({ effectParams, width, height, localTime, duration }),
	}));
}
