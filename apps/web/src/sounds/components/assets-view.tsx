	...
	useEffect(() => {
		...
		const fetchTopSounds = async () => {
			try {
				...
				if (!shouldIgnore) {
					// The online sound library is optional. Do not surface a developer
					// error overlay when its third-party service is unavailable.
					console.warn("Online sound library is unavailable:", error);
					setError({
						error:
