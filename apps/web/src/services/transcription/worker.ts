...
// The packaged desktop app ships the small offline model. Keeping remote
// loading disabled makes caption generation work without proxy configuration.
...
async function handleInit({ modelId }: { modelId: string }) {
	...
	try {
		transcriber = (await pipeline("automatic-speech-recognition", modelId, {
			dtype: "q4",
			// WebGPU initialization can hang indefinitely on some Windows drivers.
			// WASM is slower, but reliable and works on every packaged desktop app.
			device: "wasm",
