...
export interface UploadAudioElement extends BaseAudioElement {
	sourceType: "upload";
	mediaId: string;
	/** The video element this audio track was created from, when separated. */
	sourceVideoElementId?: string;
}
...
/** A visual transition attached to one side of a timeline clip. */
export interface ClipTransition {
	id: string;
	name: string;
	/** A cut joins two clips; in/out are one-sided effects on a single clip. */
	kind?: "cut" | "in" | "out";
	duration: MediaTime;
	outDuration?: MediaTime;
	inDuration?: MediaTime;
	/** State from before the transition, used to remove it without touching other edits. */
	restoreAnimations?: ElementAnimations;
	restoreStartTime?: MediaTime;
}
