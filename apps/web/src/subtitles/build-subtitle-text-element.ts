...
function wrapSubtitleText({
	...
	for (const paragraph of paragraphs) {
		...
		// Keep Latin words intact while allowing CJK text to wrap character by
		// character. This avoids both an overflowing Chinese line and a broken
		// English word such as "subtit" / "les".
		const units =
			trimmedParagraph.match(
				/[\u3400-\u9fff]|[A-Za-z0-9]+(?:['-][A-Za-z0-9]+)*|\s+|./gu,
			) ?? [trimmedParagraph];
		let currentLine = "";
		const lines: string[] = [];

		for (const unit of units) {
			const isWhitespace = /^\s+$/u.test(unit);
			const candidate = `${currentLine}${unit}`;
			if (
				currentLine &&
				measureLineWidth({ ctx, text: candidate }) > maxWidth
			) {
				lines.push(currentLine.trimEnd());
				currentLine = isWhitespace ? "" : unit.trimStart();
				continue;
			}
