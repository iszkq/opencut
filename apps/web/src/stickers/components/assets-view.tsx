...
function StickersContentView() {
	...
	if (viewMode === "search") {
		...
		if (selectedCategory !== "all" && searchQuery) {
			return <EmptyView message={`没有找到“${searchQuery}”相关贴纸`} />;
		}
	}
...
function StickerSection({
	...
	const isEmojiPack = section.items[0]?.provider === "chinese-emojis";
	const visibleItems = isEmojiPack
		? section.items.slice(0, visibleCount)
		: section.items;
	const hasHiddenItems = isEmojiPack && visibleItems.length < section.items.length;
	...
	return (
		<div className="flex flex-col gap-2">
			...
			{section.layout === "row" ? (
				<StickerRow items={visibleItems} />
			) : (
				<StickerGrid items={visibleItems} />
			)}
			...
			{hasHiddenItems && (
				...
				>
					加载更多（还剩 {section.items.length - visibleItems.length} 个）
				</Button>
			)}
		</div>
	);
