...
function compareVersions(left, right) {
	...
	if (
		leftParts.some((part) => part === null) ||
		rightParts.some((part) => part === null)
	)
		return null;
		...
		const request = https.get(
			...
			(response) => {
				...
				if (
					...
					response.resume();
					resolve(
						requestUrl(new URL(response.headers.location, url), redirects + 1),
					);
					return;
				}
				resolve(response);
...
async function checkForUpdates({ parentWindow }) {
	...
	try {
		...
		if (
			release.draft ||
			release.prerelease ||
			compareVersions(version, app.getVersion()) !== 1
		)
			return;
		const installer = Array.isArray(release.assets)
			? release.assets.find((asset) =>
					/^OpenCut-Setup-.+-x64\.exe$/i.test(String(asset.name || "")),
				)
			: null;
...
async function selectEncoder(ffmpeg) {
	try {
		...
		if (/\bh264_nvenc\b/.test(available))
			return ["h264_nvenc", "-preset", "p5", "-cq", "21"];
		if (/\bh264_qsv\b/.test(available))
			return ["h264_qsv", "-preset", "medium", "-global_quality", "21"];
		if (/\bh264_amf\b/.test(available))
			return ["h264_amf", "-quality", "balanced", "-qp_i", "21"];
	} catch {
...
async function runWatermarkRemoval(payload) {
	...
	try {
		const extension = path
			.extname(String(payload?.fileName || ""))
			.toLowerCase();
		const inputPath = path.join(
			directory,
			`input${/^[.][a-z0-9]{1,8}$/.test(extension) ? extension : ".mp4"}`,
		);
		...
		if (smart) {
			await fsp.writeFile(
				...
				createMask({
					...
					x: paddedX,
					y: paddedY,
					maskWidth: paddedWidth,
					maskHeight: paddedHeight,
				}),
			);
		}
...
ipcMain.handle("opencut:remove-watermark", async (_event, payload) =>
	runWatermarkRemoval(payload),
);
...
function getDesktopRecognizer() {
	...
	const sherpa = require(
		path.join(getWebRoot(), "node_modules", "sherpa-onnx"),
	);
...
function findDesktopSpeechRanges(samples, sampleRate) {
	...
	const threshold = Math.max(
		0.0035,
		noiseFloor + (speechLevel - noiseFloor) * 0.2,
	);
	const ranges = [];
	let speechStart = -1;
	let silenceStart = -1;
	...
	for (let frame = 0; frame < levels.length; frame += 1) {
		...
		if (frame - silenceStart + 1 >= 4) {
			addRange(
				speechStart,
				Math.min(samples.length, (silenceStart + 5) * frameSize),
			);
			speechStart = -1;
			silenceStart = -1;
		}
...
async function transcribeDesktopAudio(payload) {
	...
	for (const range of findDesktopSpeechRanges(wave.samples, wave.sampleRate)) {
		...
		stream.acceptWaveform(
			wave.sampleRate,
			wave.samples.slice(range.start, range.end),
		);
		...
		if (text)
			segments.push({
				text,
				start: range.start / wave.sampleRate,
				end: range.end / wave.sampleRate,
			});
	}
	return {
...
ipcMain.handle("opencut:transcribe-audio", async (_event, payload) =>
	transcribeDesktopAudio(payload),
);
...
const CLOUD_TTS_CONFIG_URL =
	"https://raw.githubusercontent.com/iszkq/opencut/main/config/cloud-tts.json";
const CLOUD_TTS_CONFIG_CACHE_MS = 5 * 60 * 1000;
...
let cloudTtsServiceConfig = { expiresAt: 0, value: null };
...
function getBundledCloudTtsApiKey() {
	try {
		...
		return Buffer.from(
			encrypted.map(
				(byte, index) => byte ^ maskBytes[index % maskBytes.length],
			),
		)
			.toString("utf8")
			.trim();
...
function getCloudTtsApiKey() {
	try {
		if (
			...
			fs.existsSync(getCloudTtsKeyPath())
		) {
...
function isUsingBundledCloudTtsKey() {
	return (
		!fs.existsSync(getCloudTtsKeyPath()) && Boolean(getBundledCloudTtsApiKey())
	);
}

function enforceSharedCloudTtsLimit(text, shared) {
	if (!shared) return;
	...
	if (
		text.length > 500 ||
		sharedCloudTtsUsage.characters + text.length > 20_000
	) {
		throw new Error(
			"免费共享配音每次最多 500 字、每天最多 20000 字；可在设置中使用自己的 API Key 获得更高额度。",
		);
	}
...
function normalizeCloudTtsEndpoint(value) {
...
async function fetchCloudTtsServiceConfig() {
	if (Date.now() < cloudTtsServiceConfig.expiresAt)
		return cloudTtsServiceConfig.value;
	let value = null;
	try {
		const response = await requestUrl(CLOUD_TTS_CONFIG_URL);
		if ((response.statusCode ?? 0) !== 200) {
			response.resume();
		...
		for await (const chunk of response) {
			length += chunk.length;
			if (length > 64 * 1024) throw new Error("Configuration is too large");
			chunks.push(chunk);
		}
		const parsed = JSON.parse(Buffer.concat(chunks).toString("utf8"));
		const endpoint =
			parsed?.enabled === true
				? normalizeCloudTtsEndpoint(parsed?.endpoint)
				: null;
		if (endpoint) value = { endpoint, shared: true };
	} catch {
		// A network or GitHub outage must not prevent people from using their
		// own locally saved API key.
	}
	cloudTtsServiceConfig = {
		expiresAt: Date.now() + CLOUD_TTS_CONFIG_CACHE_MS,
		value,
	};
	return value;
}

async function resolveCloudTtsService() {
	const sharedService = await fetchCloudTtsServiceConfig();
	if (sharedService) return sharedService;
	const apiKey = getCloudTtsApiKey();
	if (!apiKey) return null;
	return {
		endpoint: CLOUD_TTS_DIRECT_ENDPOINT,
		apiKey,
		shared: isUsingBundledCloudTtsKey(),
	};
}
...
async function hasTtsModel() {
	...
	try {
		...
		return (
			fs.existsSync(path.join(directory, "model.int8.onnx")) ||
			fs.existsSync(path.join(directory, "model.onnx"))
		);
	} catch {
		return false;
	}
...
async function downloadToFile({ url, destination }) {
	...
	if ((response.statusCode ?? 0) !== 200) {
		response.resume();
		throw new Error(
			`离线音色包下载失败（HTTP ${response.statusCode ?? "未知"}）。`,
		);
	}
	await new Promise((resolve, reject) => {
...
async function findKokoroModelDirectory(directory, depth = 0) {
	...
	try {
		...
		if (
			(names.has("model.int8.onnx") || names.has("model.onnx")) &&
		...
		for (const entry of entries) {
			...
			const found = await findKokoroModelDirectory(
...
async function downloadDesktopTtsModel(payload) {
	...
	ttsDownloadPromise = (async () => {
		...
		const temporaryDirectory = await fsp.mkdtemp(
			path.join(app.getPath("temp"), "opencut-tts-"),
		);
		try {
			...
			if (lastError)
				throw new Error("音色包下载失败，请检查资源镜像或网络后重试。", {
					cause: lastError,
				});
			await execFileAsync(
				"tar.exe",
				["-xjf", archivePath, "-C", extractedDirectory],
				{
					windowsHide: true,
					maxBuffer: 4 * 1024 * 1024,
				},
...
async function generateDesktopTts(payload) {
	...
	if (!audio?.samples?.length || !audio.sampleRate)
		throw new Error("没有生成可用音频，请换一个角色音色后重试。");
...
async function generateCloudTts(payload) {
	...
	if (!voice || voice.length > 100)
		throw new Error("请选择或输入有效的音色 ID。");
	const service = await resolveCloudTtsService();
	if (!service)
		throw new Error("共享配音服务暂未配置，请先保存自己的 AIHUBMIX API Key。");
	enforceSharedCloudTtsLimit(text, service.shared);

	const temporaryDirectory = await fsp.mkdtemp(
		path.join(app.getPath("temp"), "opencut-cloud-tts-"),
	);
	try {
		...
		if (service.apiKey)
			args.push("--header", `Authorization: Bearer ${service.apiKey}`);
		args.push(
			"--header",
		...
		if (bytes.subarray(0, 1).toString("utf8") === "{") {
			...
			if (process.platform === "win32" && (await isLocalProxyAvailable()))
				downloadArgs.push("--proxy", "http://127.0.0.1:7897");
			downloadArgs.push("--output", responsePath, audioUrl);
			try {
				await execFileAsync("curl.exe", downloadArgs, {
					windowsHide: true,
					maxBuffer: 1024 * 1024,
				});
			} catch {
		...
		if (bytes.length < 256 || bytes.length > 100 * 1024 * 1024)
			throw new Error("AIHUBMIX 未返回可用音频。");
...
ipcMain.handle("opencut:tts-download-model", async (_event, payload) =>
	downloadDesktopTtsModel(payload),
);
ipcMain.handle("opencut:tts-generate", async (_event, payload) =>
	generateDesktopTts(payload),
);
ipcMain.handle("opencut:tts-cloud-status", async () => {
	const service = await resolveCloudTtsService();
	return {
		configured: Boolean(service),
		shared: Boolean(service?.shared),
		secureStorage: safeStorage.isEncryptionAvailable(),
	};
});
ipcMain.handle("opencut:tts-cloud-save-key", async (_event, payload) =>
	saveCloudTtsApiKey(payload?.apiKey),
);
...
async function convertNativeMedia({
	inputPath,
	profileId,
	quality = "balanced",
	width = 0,
	...
	if (!conversionSources.has(inputPath) || !fs.existsSync(inputPath))
		throw new Error("请选择要转换的本地文件。");
	...
	const filter =
		profile.kind !== "audio"
			? conversionScaleFilter(safeWidth, safeHeight, scaleMode)
			: null;
	if (filter) args.push("-vf", filter);
	if (profile.kind === "video" && integer(fps, 1, 240))
	...
	if (profile.kind === "audio") {
		...
		if (profileId === "webp_anim")
			args.push(
				"-quality",
				quality === "high" ? "90" : quality === "small" ? "65" : "78",
			);
	} else {
		args.push("-c:v", profile.video, "-c:a", profile.audio);
		if (profile.video === "libx264" || profile.video === "libx265")
			args.push("-preset", "medium", "-crf", String(qualityConfig));
		if (profile.video === "libvpx-vp9")
			args.push("-crf", String(qualityConfig + 7), "-b:v", "0", "-row-mt", "1");
		if (profile.video === "libsvtav1")
			args.push("-crf", String(qualityConfig + 9), "-preset", "8");
		if (["libx264", "libx265"].includes(profile.video))
			args.push("-pix_fmt", "yuv420p");
		args.push(
			"-b:a",
			quality === "small" ? "128k" : quality === "high" ? "256k" : "192k",
		);
	}
	...
	if (profile.kind === "video" && duration) {
		const totalKbps = Math.max(
			64,
			Math.floor((requestedSize * 8192 * 0.98) / duration),
		);
		const audioKbps = Math.min(192, Math.max(48, Math.floor(totalKbps * 0.14)));
		args.push(
			"-b:v",
			`${Math.max(16, totalKbps - audioKbps)}k`,
			"-maxrate",
			`${Math.max(24, Math.floor((totalKbps - audioKbps) * 1.35))}k`,
			"-bufsize",
			`${Math.max(32, (totalKbps - audioKbps) * 2)}k`,
			"-b:a",
			`${audioKbps}k`,
		);
	}
	...
	if (["mp4", "mov", "m4a"].includes(profile.extension))
...
async function exportNativeVideo(payload) {
	...
	const directory = await fsp.mkdtemp(
		path.join(os.tmpdir(), "opencut-native-export-"),
	);
	try {
		const ffmpeg = getFfmpegPath();
		const metadata = await Promise.all(
			clips.map((clip) => probeNativeClip(clip.sourcePath)),
		);
		...
		const concatInputs = clips
			.map((_, index) =>
				payload?.includeAudio ? `[v${index}][a${index}]` : `[v${index}]`,
			)
			.join("");
		const filter = [
			...videoFilters,
...
ipcMain.handle("opencut:export-native-video", async (_event, payload) =>
	exportNativeVideo(payload),
);
...
async function exportRenderedVideo(payload) {
	...
	if (save.canceled || !save.filePath) return { cancelled: true };
	const directory = await fsp.mkdtemp(
		path.join(os.tmpdir(), "opencut-rendered-export-"),
	);
	try {
...
ipcMain.handle("opencut:export-rendered-video", async (_event, payload) =>
...
async function getAvailableWebPort(preferredPort) {
	...
	throw new Error(
		"已有 OpenCut 正在运行。请先关闭所有旧版 OpenCut 窗口，再重新打开本应用；你的项目不会丢失。",
	);
}
...
async function createWindow() {
	...
	mainWindow.webContents.on(
		...
		(_event, errorCode, errorDescription, validatedUrl, isMainFrame) => {
			...
			mainWindow?.loadURL(
				`data:text/html;charset=utf-8,${encodeURIComponent(`<main style="font-family:system-ui;padding:48px"><h2>打开编辑器失败</h2><p>${message}</p></main>`)}`,
			);
		},
	);
	if (!app.isPackaged || process.env.OPENCUT_DEBUG === "1") {
...
if (!hasSingleInstanceLock) {
	...
	app
		...
		.then(async () => {
			...
			setTimeout(
				() => void checkForUpdates({ parentWindow: mainWindow }),
				5_000,
			);
		})
		.catch((error) => {
