		...
		if (isSourceAudioSeparated({ element: videoElement })) {
			editor.timeline.updateTracks(
				updateSourceAudioEnabled({
					tracks: removeSeparatedAudio({
						tracks: this.savedState,
						sourceVideoElementId: videoElement.id,
					}),
...
function removeSeparatedAudio({
	tracks,
	sourceVideoElementId,
}: {
	tracks: SceneTracks;
	...
	return {
		...
		audio: tracks.audio.map((track) => ({
			...
			elements: track.elements.filter(
				(element) => element.sourceVideoElementId !== sourceVideoElementId,
			),
		})),
	};
}
